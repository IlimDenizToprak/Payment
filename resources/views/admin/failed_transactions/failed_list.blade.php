                            ...
                            <div class="table-responsive mb-4 mt-4">
                                <table id="html5-extension" class="table table-hover non-hover" style="width:100%">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{$failed_user->id}}</td>
                                            <td>{{$failed_user->Users->name}}</td>
                                            <td>{{$failed_user->Users->email}}</td>
                                            <td>
                                                @if ($failed_user->Users->status == 1)
                                                    <span class="badge badge-success"> Active </span>
                                                @else
                                                    <span class="badge badge-danger"> Passive </span>
                                                @endif
                                            </td>
                                            <td>{{ \Carbon\Carbon::parse($failed_user->Users->start_date)->format('d-m-Y H:i') }}</td>
                                            <td>{{ \Carbon\Carbon::parse($failed_user->Users->end_date)->format('d-m-Y H:i') }}</td>
                                            <td>{{ \Carbon\Carbon::parse($failed_user->last_try)->format('d-m-Y H:i') }}</td>
                                            <td>{{$failed_user->failed_try}} time</td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                                </table>
